...
export function ResultsPanel(props: ResultsPanelProps) {
    ...
    const rawPayload = useMemo(
        () => JSON.stringify(tool === 'search' ? searchResult : researchResult, null, 2),
        [researchResult, searchResult, tool]
    );
    ...
    const collapsedBodyClass = 'custom-scrollbar h-[42rem] overflow-y-auto overscroll-contain';
        ...
        <div className="relative min-h-[700px] overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white xl:col-span-8">
            ...
            <div className="relative min-h-[640px] bg-white">
                ...
                {hasResult && !isPending && !error && (
                    viewRaw ? (
                        <div className="p-8">
                            <pre className={`custom-scrollbar overflow-auto overscroll-contain rounded-[1.5rem] border border-slate-200 bg-white p-5 text-sm leading-6 text-slate-700 ${expanded ? 'max-h-none' : 'h-[42rem]'}`}>
                                <code>{rawPayload}</code>
                            </pre>
                        </div>
                    ) : tool === 'search' ? (
                        <div className={`space-y-5 p-8 ${expanded ? '' : collapsedBodyClass}`}>
                            {searchResult?.answer && (
                                ...
                                <div className="space-y-4">
                                    {searchResult.results.map((result, index) => (
                                        <div key={`${result.url}-${index}`} className="rounded-[1.5rem] border border-slate-200 bg-white p-5">
                                            ...
                                            {result.rawContent && (
                                                <pre className="custom-scrollbar mt-4 overflow-auto rounded-2xl border border-slate-200 bg-slate-50 p-4 text-xs leading-6 text-slate-700">
