...
export function ResultsHeader(props: ResultsHeaderProps) {
    ...
    const statusLabel = hasError ? 'Failed' : isPending ? 'Extracting' : hasResult ? 'Ready' : 'Idle';
    ...
    return (
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-slate-50 px-6 py-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
                ...
                {canSave && (
                    <button type="button" disabled={isSaving} onClick={onSave} className="inline-flex items-center gap-1.5 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-xs font-semibold text-emerald-700 transition-colors hover:bg-emerald-100 disabled:cursor-not-allowed disabled:opacity-70">
                        <HugeiconsIcon icon={isSaving ? Loading03Icon : Download01Icon} className={`h-3.5 w-3.5 ${isSaving ? 'animate-spin' : ''}`} />
