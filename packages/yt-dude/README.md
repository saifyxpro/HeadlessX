...
`yt-dude` is a HeadlessX-maintained fork of `yt-dlp` for resilient YouTube and media extraction in automation-heavy environments.

It keeps the familiar `yt-dlp` command-line workflow, but the fork is maintained around HeadlessX use cases: background workers, long-running scraping jobs, lower-friction client spoofing, and fewer breakages from the default desktop YouTube path.

## Fork Notice

- upstream base: `yt-dlp`
- fork name: `yt-dude`
- maintained by: `HeadlessX`
- package entrypoint: `yt-dude`

This repository is not a clean rebrand. It is a maintained fork intended to carry HeadlessX-focused extraction and reliability changes while preserving broad `yt-dlp` compatibility.

## Why This Fork Exists

`yt-dude` exists to support production-style extraction workloads where default upstream behavior is not always ideal for:

- queue workers
- Celery-style background jobs
- API-driven download platforms
- long-lived automation services
- lower-friction YouTube client impersonation

## Key Changes In yt-dude

- **Forked from yt-dlp**: keeps the familiar CLI and embedding model while carrying fork-specific extractor and reliability changes.
- **Maintained by HeadlessX**: intended to fit the HeadlessX ecosystem and adjacent automation packages.
- **YouTube anti-throttling defaults**: prefers mobile-style client behavior for more reliable extraction in automated environments.
- **Worker-safe timing defaults**: includes safer sleep behavior for repeated background execution.
- **Extractor hotfixes**: carries targeted fixes that may not exist upstream at the same time.
- **Automation-focused maintenance**: tuned for server and worker usage, not just one-off local downloads.

## Quick Start

Install from source in this workspace:

```bash
cd /home/saifyxpro/CODE/Crawl/HeadlessX/packages/yt-dude
...
```bash
yt-dude --help
yt-dude "https://www.youtube.com/watch?v=VIDEO_ID"
```

Recommended Python baseline from this package:

- Python `>=3.10`

## Recommended Runtime Setup

For full YouTube support, this fork expects the same practical tooling you would want for serious `yt-dlp` usage:

- `ffmpeg`
- `ffprobe`
- `yt-dude-ejs`
- a JavaScript runtime such as `deno`, `node`, `bun`, or `QuickJS`

## Notes For HeadlessX Users

- `yt-dude` is the media extraction fork inside the wider HeadlessX package direction.
- This package is separate from the HeadlessX API and `headlessx-cli`.
- The goal is to keep the extractor layer reusable on its own, while still being maintained for HeadlessX workloads.
