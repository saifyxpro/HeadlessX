...
function getPrimaryConfigDir(): string {
  ...
  switch (os.platform()) {
    case 'darwin':
      return path.join(homeDir, 'Library', 'Application Support', 'headlessx');
    case 'win32':
      return path.join(homeDir, 'AppData', 'Roaming', 'headlessx');
    default:
      return path.join(homeDir, '.config', 'headlessx');
  }
}

function getHxCliConfigDir(): string {
  const homeDir = os.homedir();
  switch (os.platform()) {
    case 'darwin':
...
export function loadCredentials(): StoredCredentials | null {
  return (
    readCredentialsFrom(getPrimaryConfigDir()) ??
    readCredentialsFrom(getHxCliConfigDir()) ??
    readCredentialsFrom(getLegacyConfigDir())
  );
}
