...
const config = program.command('config').description('View or update local hx-cli configuration.');
...
program
  .command('map')
  .description('Discover links on a site through /api/website/map.')
...
program
  .command('google')
  .description('Run a Google SERP scrape through HeadlessX.')
